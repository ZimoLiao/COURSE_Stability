...
constexpr long double Re = 5000, alpha = 0.179;
constexpr complex<long double> iunit = {0.0, 1.0};
long double unit = 1.0;

// r.h.s. of blasius equation
void f_blasius(const long double var[3],
               long double f[3]);

// r.h.s. of compound matrix for O-S equation
void f_compound(const complex<long double> c, const long double u, const long double ddu,
                const complex<long double> var[6],
                complex<long double> f[6]);
...
int main()
{
    ...
    int ny = 10000;
    long double ly = 60, dy = ly / ny;

    long double *y = new long double[ny];
    long double *u = new long double[ny];
    long double *ddu = new long double[ny];
    for (int i = 0; i != ny; i++)
    {
        y[i] = dy * i;
        u[i] = 0.0;
    ...
    {
        long double var[3] = {0.0, 0.0, 0.332057336}, varn[3], // TODO: Blasius solution
            k1[3], k2[3], k3[3], k4[3];
    ...
    // parameters
    // long double Re = 580, alpha = 0.179;
    complex<long double> c;

    // shooting grid initialization
    int ncr = 1000, nci = 1000;
    long double crmin = 0.0, crmax = 1.0, cimin = -0.8, cimax = 0.1;
    long double *cr, *ci;
    cr = new long double[ncr];
    for (int r = 0; r != ncr; r++)
        cr[r] = crmin + r * (crmax - crmin) / (ncr);
    ci = new long double[nci];
    for (int i = 0; i != nci; i++)
        ci[i] = cimin + i * (cimax - cimin) / (nci - 1);

    long double **Dr, **Di;
    Dr = new long double *[ncr];
    Di = new long double *[ncr];
    for (int r = 0; r != ncr; r++)
    {
        Dr[r] = new long double[nci];
        Di[r] = new long double[nci];
    }

    // solve
    {
        complex<long double> p, Y[6], Yn[6],
            k1[6], k2[6], k3[6], k4[6];
        ...
        for (int r = 0; r < ncr; r++)
        {
            for (int i = 0; i < nci; i++)
            {
                c = complex<long double>(cr[r], ci[i]);
                ...
                complex<long double> coef = (alpha - p) * exp(-(alpha + p) * ly);
                Y[0] *= coef;
                Y[1] *= coef;
                Y[2] *= coef;
                Y[3] *= coef;
    ...
    // write D
    {
        ...
        for (int i = 0; i != nci; i++)
        {
            for (int r = 0; r != ncr; r++)
            {
                long double D = sqrt(Dr[r][i] * Dr[r][i] + Di[r][i] * Di[r][i]);
                fout << setw(8) << cr[r] << '\t'
                     << setw(8) << ci[i] << '\t'
                     << setw(12) << Dr[r][i] << '\t'
                     << setw(12) << Di[r][i] << '\t'
